		...
		public void StartDatabase()
		{
			...
			if (_pronosContestContextDatabase.Competitions.Count() == 0)
			{
                ...
                if (user != null)
                {
                    ...
                    var match_huitieme_8 = new Match() { NumeroMatch = 44, EquipePossibleDomicile_Libelle = "1° Gr. B", EquipePossibleDomicile_Groupes = new JavaScriptSerializer().Serialize(new List<char> { 'B' }), EquipePossibleDomicile_Place = 1, EquipePossibleExterieur_Libelle = "2° Gr. F", EquipePossibleExterieur_Groupes = new JavaScriptSerializer().Serialize(new List<char> { 'F' }), EquipePossibleExterieur_Place = 2, Date = new DateTime(2016, 6, 27, 21, 00, 00), Stade = "Stade Allianz Riviera, Nice" };
